...
flights_Jan11th <- read.csv('Time-Series Jan 10th, 2023.csv')
str(flights_Jan11th)
flights_Jan11th$Date <- as.Date(flights_Jan11th$Date, 
                                     format = "%d/%m/%Y")
str(flights_Jan11th)
...
# Define a function to forecast and plot the SARIMA model
forecast_and_plot_Jan11th_and_12th <- function(variable_name) {
  # Create a time series object
  ts_data_4_Jan11th <- ts(flights_Jan11th[[variable_name]], frequency = 7)
  # Call the function
  forecast_result <- sarima(ts_data_4_Jan11th)
  # Calculate the start and end indices for the 7 days before the 1st forecasted day
  start_index <- length(ts_data_4_Jan11th) - 6
  end_index <- length(ts_data_4_Jan11th)
  # Extract the desired range of dates
  ts_data_subset <- ts_data_4_Jan11th[start_index:end_index]
  # Subset the forecast_result
  forecast_result_subset <- forecast_result
  forecast_result_subset$x <- tail(forecast_result$x, 9)
  forecast_result_subset$mean <- tail(forecast_result$mean, 9)
  ...
  # Create date labels for the x-axis
  date_labels <- as.character(seq(tail(flights_Jan11th$Date, 1) - 6, 
                                  tail(flights_Jan11th$Date, 1) + 2, by = "1 day"))
  # Create date labels for the x-axis
  custom_date_breaks <- as.Date(c("2023-01-06", "2023-01-07", "2023-01-08", "2023-01-09", "2023-01-10", "2023-01-11", "2023-01-12"))
  custom_date_labels <- date_format("%b %d")(custom_date_breaks)
  ...
  # Plot the forecast
  plot_Jan11th_12th <- ggplot() +
    geom_line(data = combined_df, aes(x = Date, y = Value)) +
    geom_line(data = forecast_df, aes(x = Date, y = Value), color = "cyan") +
    geom_text(data = combined_df, aes(x = Date, y = Value, label = round(Value, 0)), vjust = -0.5, size = 3) +
    ggtitle(paste("SARIMA Forecasts for January 11th & 12th of 2023:", variable_name)) +
    ylab("Flights Per Day") + xlab(" ") +
    scale_x_date(breaks = custom_date_breaks, labels = custom_date_labels, limits = as.Date(c("2023-01-06", tail(flights$Date, 1) + 2))) +
    scale_y_continuous(expand = expansion(mult = c(0.4, 0.4))) + theme_minimal() +
    theme(panel.grid.minor.x = element_blank(),
          panel.grid.minor.y = element_blank() )
  
  return(plot_Jan11th_12th) }

# Create an empty list to store the plots
plots <- list()

for (variable_name in variable_names) {
  plot_Jan11th_12th <- forecast_and_plot_Jan11th_and_12th(variable_name)
  plots[[variable_name]] <- plot_Jan11th_12th}
# Display the plots
grid.arrange(plots[["Fixed.Wing.Multi.Engine"]], 
             plots[["Fixed.Wing.Single.Engine"]], 
             plots[["Rotorcraft"]], nrow = 3)
